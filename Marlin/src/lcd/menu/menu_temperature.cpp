...
int nozzle2=0;//fore dual_ex
void menu_temperature() {
  START_MENU();
  MENU_BACK(MSG_MAIN);
  #if ENABLED(DUAL_EX)
  nozzle2=analogRead(A3);//fore dual_ex
  #endif
  //
  // Nozzle:
  // Nozzle [1-5]:
  //
    ...
    //dual_ex tempereature engellemek için değişen kısım
    #if ENABLED(DUAL_EX)
    if(65<nozzle2 && nozzle2<125)
    { EDIT_TARGET(0);
      EDIT_TARGET(1);}
      else{
          MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, MSG_NOZZLE, &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15, watch_temp_callback_E0);
      }
    #endif// //dual_ex tempereature engellemek için değişen kısım
    /*orjinali aşağıdaki gibiydi
    #define EDIT_TARGET(N) MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, MSG_NOZZLE MSG_LCD_N##N, &thermalManager.target_temperature[N], 0, HEATER_##N##_MAXTEMP - 15, watch_temp_callback_E##N)
    EDIT_TARGET(0);
    EDIT_TARGET(1);
    */
  ...
  //
    ...
    //deltafab tarafından kaldırıldı
