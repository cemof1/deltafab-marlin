...
int baraj_fan=0;
int okuma_fan=0;
int sayac_debug_fan=0;// kafa takılıyken genede kafa algılanmadı mesajı bugını gidermek iıin sayaç eklendi
void baslik_okuma_fan(){
if(baraj_fan==0){// 10 kez okuma yapıp sabitleme kısmı
  for(sayac_fan=0; sayac_fan<10 ; sayac_fan++){
    okuma_fan=analogRead(A3);
    delay(5);
    toplam_fan=okuma_fan+toplam_fan;
    }
  
  }
if(0<(toplam_fan/10) && (toplam_fan/10)<60){baslik_fan=1;} // seramik
else if(65<(toplam_fan/10) && (toplam_fan/10)<125){baslik_fan=2;} // çift nozzle
else if(130<(toplam_fan/10) && (toplam_fan/10)<190){baslik_fan=3;} // lazer
else if(195<(toplam_fan/10) && (toplam_fan/10)<255){baslik_fan=4;}// cnc
/*
 *      .
 *      ..  
 *      ...
...
void Temperature::manage_heater() {
  #if ENABLED(DELTAFAB_SMART_FAN)
  baslik_okuma_fan();
  if(baslik_fan!=2){if( current_temperature[0]>34){digitalWrite(7, HIGH);}
      else{digitalWrite(7,LOW);}}
  else{if(current_temperature[0]>34){digitalWrite(5, HIGH);}else{digitalWrite(5,LOW);}
        if(current_temperature[1]>34){digitalWrite(5, HIGH);}else{digitalWrite(5,LOW);}
  }
  #endif
  #if EARLY_WATCHDOG
  ...
  HOTEND_LOOP() {
    #if ENABLED(DELTAFAB_SMART_FAN)
    //if(baslik!=2){if(current_temperature[e]>50){digitalWrite(7,HIGH);}}
    #endif
    #if HEATER_IDLE_HANDLER
...
void Temperature::set_current_temp_raw() {
  //if(current_temperature[HOTENDS]>40){digitalWrite(8,HIGH);}//FAN DENEME
