...
baslik_okuma();
  ...
  if(baslik!=2){if( Temperature::current_temperature[0]>34){digitalWrite(7, HIGH);}
      else{digitalWrite(7,LOW);}}
  else{if(Temperature::current_temperature[0]>34){digitalWrite(5, HIGH);}else{digitalWrite(5,LOW);}
        if(Temperature::current_temperature[1]>34){digitalWrite(5, HIGH);}else{digitalWrite(5,LOW);}
  }
  #endif
    #if ENABLED(LASER_PAUSE)
    if(baslik==3 && card.flag.sdprinting==false) {PSTR("M106 P10");}
    else if(baslik==3 && card.flag.sdprinting==true && digitalRead(58)==1){card.pauseSDPrint(); PSTR("M106 P10\nM117 Door Opened");}
    #endif

    #if ENABLED(DELTAFAB_KAFA_ALGILAMA)
    kafa_algilama();
    #endif
    #if ENABLED(SDSUPPORT)
      card.checkautostart();
