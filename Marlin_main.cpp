...
/*if (baslik==3){
  ...
  {//lazerde durdurma
    ...
    LCD_MESSAGEPGM(MSG_PAUSED_LASER);
    }
  }*/
}
...
void home_all_axes() { gcode_G28(true); }
 ...
 /*int seramik=A3;//Added for ceramic headboard
 pinMode(seramik, INPUT);  //Added for ceramic headboard
 int deger=analogRead(seramik);*/
      if(baslik==1){       //Added for ceramic headboard
      UNUSED(stow);
      UNUSED(set_up);
      return lcd_probe_pt(nx, ny);}
      else{                               //Added for ceramic headboard
...
inline void gcode_M17() {
  ...
  inline void gcode_M25() {
    card.pauseSDPrint();
    print_job_timer.pause();
    LCD_MESSAGEPGM(MSG_PAUSE_PRINT);
    #if ENABLED(PARK_HEAD_ON_PAUSE)
...
//wifi denemesi
#if ENABLED(Deltafab_wifi)
  #ifndef INVERT_WIFI
     #define INVERT_WIFI false
  #endif
  
  bool deltafab_wifi_on=true;
      void update_deltafab_wifi(){
      
      SET_OUTPUT(deltafab_wifi_PIN); 
        const bool d = deltafab_wifi_on ? !INVERT_WIFI : INVERT_WIFI;
        WRITE(deltafab_wifi_PIN, d ? HIGH : LOW);
   //    if(deltafab_wifi_on==true){analogWrite(deltafab_wifi_PIN, 200);}
    //   else
   //    {digitalWrite(deltafab_wifi_PIN, LOW);}
     }
#endif
...
inline void gcode_M355() {
    ...
    SERIAL_ECHO_START();
    if (!case_light_on) {
      SERIAL_ECHOLNPGM("wifi: off"); //Added for wifi
    }
    else {
      if (!USEABLE_HARDWARE_PWM(CASE_LIGHT_PIN)) SERIAL_ECHOLNPGM("wifi: on"); //Added for wifi
...
/*
 * Deltafab Code D11: printer is stoped
 */
inline void gcode_D11() {    
  card.stopSDPrint();
  LCD_MESSAGEPGM(MSG_STOP_PRINT);
  MYSERIAL0.println(MSG_STOP_PRINT);
}
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      case 998: gcode_M998(); break;                              // M998: Seramik kalibrasyon g kodu
      case 999: gcode_M999(); break;                              // M999: Restart after being Stopped
      case 1000: gcode_D11(); break;
      default: parser.unknown_command_error();
    }
    break;
